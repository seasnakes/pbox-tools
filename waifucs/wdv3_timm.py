...
def main(opts: ScriptOptions):
    ...
    for image_type in image_types:
        files = list(image_dir.glob(image_type))
        for file in files:
        # 检查文件名是否以 . 开头
            if not file.name.startswith("."):
                image_files.append(file)
    
    image_files = tqdm(image_files, total=len(image_files))
    for image_file in image_files:
        print(f"Processing image file: {image_file}")
        tag_file = image_file.with_suffix('.txt')   # 获取tag文件的名字
        if os.path.isfile(tag_file):   # 检查tag文件是否已经存在
            print(f"Tag file '{tag_file}' already exists. Skipping image file: {image_file}")
            continue   # 如果tag文件已经存在，跳过这个图像文件
        ...
        tag_file = image_file.with_suffix('.txt')
